...
import xyz.welt.bean.vo.AdminRoutersVo;
import xyz.welt.bean.vo.AdminUserInfoVo;
...
public class AdminLoginController {
    ...
    public ResponseResult<AdminUserInfoVo> getInfo(){
        ...
        //根据用户ID查询权限信息
        List<String> parms =  menuService.selectPermsByUserId(loginUser.getUser().getId());
        //根据用户id查询角色信息
        ...
        //封装数据返回
        AdminUserInfoVo adminUserInfoVo = new AdminUserInfoVo(parms,roleKeyList,userInfoVo);

        return ResponseResult.okResult(adminUserInfoVo);
    }

    @GetMapping("/getRouters")
    public ResponseResult<AdminRoutersVo> getRouters(){
        //获取当前登录的用户Id
        Long userId = SecurityUtils.getUserId();
        //查询menu 结果是tree的形式
        List<Menu> menus = menuService.selectRouterMenuTreeByUserId(userId);
        //封装数据返回
        return ResponseResult.okResult(new AdminRoutersVo(menus));
    }
